...
const markdownFiles = import.meta.glob("../data/markdown/studyProject_*.md", {
  as: "raw",
});
...
export const StudyProject: React.FC = () => {
  const { studyProjectId } = useParams();
  const studyProject = studyProjects.find((sp) => sp.id === studyProjectId);
  const [markdown, setMarkdown] = useState("");

  useEffect(() => {
    if (studyProject) {
        const path = `../data/markdown/studyProject_${studyProject.id}.md`;
  ...
  if (!studyProject)
    return (
      <div className="flex items-center justify-center min-h-screen bg-zinc-950 text-gray-300">
        <p className="text-2xl animate-pulse">Study Project not found...</p>
      </div>
    );
  ...
  return (
    ...
    <div className="min-h-screen font-inter text-gray-100 relative main-app-container">
      ...
      <div
        ...
        style={{
          backgroundImage: `url(${studyProject.coverImage})`,
      ...
      >
        ...
        <div className="relative z-10 text-center max-w-6xl w-full">
          <img
            src={studyProject.thumbnail}
            alt={studyProject.name}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          <InfoCard title="Description" content={studyProject.description} />
        </div>
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {studyProjects.map(() => (    
              <a
                key={studyProject.relatedPosts[0]}
                href={`/blog/${studyProject.relatedPosts[0]}`}
                onClick={(e) => { e.preventDefault(); console.log(`Navigating to simulated blog post: ${studyProject.relatedPosts[0]}`); }}
              ...
              >
                <h3 className="text-xl font-semibold text-white transition-colors duration-300" style={{ color: `#307167` }} onMouseEnter={(e) => e.currentTarget.style.color = `#307167`} onMouseLeave={(e) => e.currentTarget.style.color = 'white'}>
                  {studyProject.relatedPosts[0].split("-").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")}
                </h3>
